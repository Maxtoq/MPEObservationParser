...
class ObservationParserStrat(ColorShapeParser):
    ...
    def __init__(self, sce_conf, obj_colors, obj_shapes, land_colors, land_shapes):
        ...
        self.map = ColorShapeMapper(sce_conf)
    ...
    def select_not_object(self, objects):
        '''
        Will select one random object from the list

        Input:  
            objects: list(list(int)) list of objects 

        Output: list(int) one object
        '''
        list_obj = []
        list_land = []
        list = []
    ...
    def not_sentence(self, i, j, nb_agent):
        ...
        if check == "North":
            ...
            objects = self.map.check_area(nb_agent, 0, 0, self.obj_colors, self.obj_shapes)
            # Then we reset the area
            self.map.reset_areas(0, nb_agent)
        elif check == "South":
            objects = self.map.check_area(nb_agent, 0, 2, self.obj_colors, self.obj_shapes)
            self.map.reset_areas(1, nb_agent)
        elif check == "West":
            objects = self.map.check_area(nb_agent, 1, 0, self.obj_colors, self.obj_shapes)
            self.map.reset_areas(2, nb_agent)
        elif check == "East":
            objects = self.map.check_area(nb_agent, 1, 2, self.obj_colors, self.obj_shapes)
        ...
        for i in range(len(objects)):
            n_sent.extend([self.get_color(objects[i][1]),self.get_shape(objects[i][2]),obj_name[objects[i][0]], \
                "Not"])
        ...
        if  obs[0] == 1 :
            ...
            for object in range(int(self.nb_objects)):
                ...
                if  obs[spot] == 1 :
                    ...
                    # If collision
                    if distance < 0.47:
                        sentence.extend(["You","Push",self.array_to_color(obs[spot+5:spot+8]),self.get_shape(obs[spot+8]),"Object"])
    ...
    def object_sentence(self, pos_agent, obs, nb_agent):
        ...
        if  obs[0] == 1 :
            ...
            self.map.update_area_obj(pos_agent[0], pos_agent[1],2,self.array_to_num(obs[5:8]),obs[8],nb_agent)
            sentence.append(self.array_to_color(obs[5:8]))
            sentence.append(self.get_shape(obs[8]))
            ...
            # If collision
            if distance < 0.47:
                sentence.extend(["I","Push",self.array_to_color(obs[5:8]),self.get_shape(obs[8]),"Object"])
                push = True
                # Calculate where the object was pushed 
                # Based on its distance from the agent
    ...
    def landmark_sentence(self, pos_agent, obs, nb_agent):
        ...
        if  obs[0] == 1 :
            ...
            #We update the area_obj
            self.map.update_area_obj(pos_agent[0], pos_agent[1],3,self.array_to_num(obs[3:6]), obs[6], nb_agent)
            sentence.append(self.array_to_color(obs[3:6]))
            sentence.append(self.get_shape(obs[6]))
    ...
    def parse_obs(self, obs, nb_agent):
        ...
        for object in range(self.nb_objects):
            obj_sent , push = self.object_sentence(pos_agent,obs[place:place+9],nb_agent)
        ...
        for landmark in range(self.nb_objects):
            landmarks_sentence.extend(self.landmark_sentence(pos_agent,obs[place:place+7],nb_agent))
            place = place + 7
    ...
    def reset(self, obj_colors, obj_shapes, land_colors, land_shapes):
        '''
        Reset the map of the agent and the colors and shapes of the exercise
        '''
        self.map.reset()
        self.obj_colors = obj_colors
        self.land_colors = land_colors
...
class ObservationParser(ColorShapeParser):
    ...
    def not_sentence(self, position, not_vis_obj, not_vis_land):
        ...
        if random.random() <= self.args.chance_not_sent:
            ...
            if not_sentence == 1 and len(not_vis_obj) != 0:
                ...
                sentence.extend([self.get_color(self.obj_colors[obj]), self.get_shape(self.obj_shapes[obj]),"Object","Not"])
                ...
                sentence.extend([self.get_color(self.land_colors[obj]), self.get_shape(self.land_shapes[obj]),"Landmark","Not"])
                for word in position:
                    sentence.append(word)
            elif not_sentence == 3:
                # Both object
    ...
    def parse_obs(self, obs):
        ...
        for object in range(self.nb_objects):
            object_sentence = []
            object_sentence.extend(self.object_sentence(obs[place:place+9]))
            place = place + 9
        ...
        for landmark in range(self.nb_objects):
            landmark_sentence = []
            landmark_sentence.extend(self.landmark_sentence(obs[place:place+7]))
            place = place + 7
            # If we don't see the object
